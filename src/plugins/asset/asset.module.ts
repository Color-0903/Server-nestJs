...
export class AssetServerPlugin implements NestModule, OnApplicationBootstrap {
  ...
  static init(options: AssetServerOptions): Type<AssetServerPlugin> {
    ...
    AssetServerPlugin.configure(defaultConfig);
    return this;
  }
  ...
  configure(consumer: MiddlewareConsumer) {
    console.log('Creating asset server middleware');
    ...
    return async (err: any, req: Request, res: Response, next: NextFunction) => {
      if (err && (err.status === 404 || err.statusCode === 404)) {
        if (req.query) {
          ...
          Logger.debug(`Pre-cached Asset not found: ${decodedReqPath}`);
          let file: Buffer;
          try {
          ...
          try {
            ...
            if (!req.query.cache || req.query.cache === 'true') {
              ...
              Logger.debug(`Saved cached asset: ${cachedFileName}`);
            }
