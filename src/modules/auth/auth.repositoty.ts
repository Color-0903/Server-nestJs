...
export const NativeAuthenticationMethodRepository = dataSource.getRepository('').extend({
  async updateInformationChangePassword(passwordHash: string, id: string, lastModifiedByUserId?: string) {
    return NativeAuthenticationMethodRepository.createQueryBuilder()
      .update()
      .set({ passwordHash: passwordHash, lastModifiedByUserId: lastModifiedByUserId })
      .where('id = :id', { id: id })
      .execute();
  },
  async updateIdentier(id: string, identifier: string, lastModifiedByUserId?: string) {
    return NativeAuthenticationMethodRepository.createQueryBuilder()
      .update()
      .set({ identifier: identifier, lastModifiedByUserId: lastModifiedByUserId })
      .where('id = :id', { id: id })
      .execute();
  },
  ...
  async deleteAuthenticationMethod(id: string, type: DELETE_TYPE) {
    return type === DELETE_TYPE.HARD
      ...
      : await NativeAuthenticationMethodRepository.update(id, { deletedAt: new Date() });
  },
});
